  ...
  def load_items
    unless @load_items
      @load_items = RTA::Transaction.new("load items") do
        ...
        sql = "INSERT INTO item (i_id, i_name, i_price, i_data) "+
              "VALUES (?, ?, ?, ?)"
        stmt = @con.prepareStatement(sql)
        stmt.setInt(1, i_id)
        stmt.setString(2, i_name)
        stmt.setFloat(3, i_price)
  ...
  def load_ware
    unless @load_ware
      @load_ware = RTA::Transaction.new("load ware") do
        ...
        w_tax = random_number(10, 20) / 100.0
        w_ytd = 3000000.00
  ...
  def load_district
    unless @load_district
      @load_district = RTA::Transaction.new("load district") do
        ...
        d_tax = random_number(10, 20) / 100.0
